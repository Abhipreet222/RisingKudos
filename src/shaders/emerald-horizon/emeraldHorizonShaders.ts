...
void main() {
  ...
  vec3 bgColor = vec3(0.878, 0.478, 0.373); // #E07A5F background
  ...
  vec3 waveColor = finalGlow * pow(intensity, 1.5) * 1.2 * u_glow;
  float vignette = mix(1.0, smoothstep(1.2, 0.5, length(st - vec2(0.5, 0.0))), u_vignette);
  
  // Apply vignette only to the wave so the background stays light orange
  waveColor *= vignette;
  
  vec3 color = bgColor + waveColor;
